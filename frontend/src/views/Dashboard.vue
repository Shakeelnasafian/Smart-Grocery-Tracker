<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 p-6">
    <div class="max-w-4xl mx-auto bg-white shadow-2xl rounded-3xl p-8 space-y-8">
      ...
      <form @submit.prevent="sesLogout" method="post">
        <button type="submit"
      ...
      <form @submit.prevent="addItem" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <input v-model="newItem.category" placeholder="Category"
          class="col-span-1 px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
        <input type="date" v-model="newItem.expiry_date"
          class="col-span-1 px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
        <button type="submit"
      ...
      <div class="space-y-4">
        <div v-for="item in items" :key="item.id"
          ...
          <div>
            <p class="text-lg font-bold text-gray-800">{{ item.name }} <span class="text-sm text-gray-500">({{
...
const sesLogout = async () => {
  try {
    ...
    auth.logout();  // Clear token on client
    router.push('/'); // Navigate to homepage/login
  }
};
