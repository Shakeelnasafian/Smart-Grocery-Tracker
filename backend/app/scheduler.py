...
def _check_and_send_alerts() -> None:
    ...
    try:
        ...
        for setting in alert_settings:
            ...
            rows = "".join(
                f"<tr><td>{i.name}</td><td>{i.category}</td><td>{i.expiry_date}</td><td>{i.quantity}</td></tr>"
                for i in expiring
            )
            body = f"""
            <html><body>
            <h2>Grocery Expiry Alert 🛒</h2>
            <p>The following items are expiring within <strong>{setting.days_before_expiry} days</strong>:</p>
            <table border="1" cellpadding="6" cellspacing="0">
              <thead><tr><th>Name</th><th>Category</th><th>Expiry Date</th><th>Quantity</th></tr></thead>
              <tbody>{rows}</tbody>
            </table>
...
def start_scheduler() -> None:
    scheduler.add_job(_check_and_send_alerts, "cron", hour=8, minute=0, id="expiry_alerts")
    scheduler.start()
